...
int main(void)
{
    ...
    while (done)
    {
        ...
        thread:直接初始化一个pthread_t thread_id;它不用做任何初始化,直接&thread_id放进去即可;它的作用是标识当前线程的id,相当于当前线程的身份证
        attr:置为NULL即可
        start_routine:第三个参数是返回值为void*的函数
        arg: 传递给start_routine函数的参数。它可以是任何类型的指针，只要将它强制转换为void*类型即可。
        */
...
// gcc minihttp.c -o minihttp -lpthread
//  http://192.168.8.129/demo.html
